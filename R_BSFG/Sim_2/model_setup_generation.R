...
setup=setup_pedigree_GeneLine(data,GenerationCode=c(0,1),LineCode=1)
...
#filenames = list.files("~/Runcie Lab/pedigree",pattern="*.RData")
#setwd("~/Runcie Lab/pedigree")
#allfiles = lapply(filenames,function(x){get(load(x))})
#names(allfiles) = sapply(filenames ,function(x)substr(x,1,(nchar(x)-6)))
