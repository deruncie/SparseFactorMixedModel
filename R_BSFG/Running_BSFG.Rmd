...
maps = make_Missing_data_map(BSFG_state)
...
(n_threads = optimize_n_threads(BSFG_state,seq(1,RcppParallel::defaultNumThreads(),by=1),times=2))
set_BSFG_nthreads(n_threads$optim)
for(i  in 1:70) {
  print(sprintf('Run %d',i))
  BSFG_state = sample_BSFG(BSFG_state,n_iter)  # run MCMC chain n_samples iterations. grainSize is a paramter for parallelization (smaller = more parallelization)
...
G_samples = get_posterior_FUN(BSFG_state,Lambda %*% diag(F_h2['animal',]) %*% t(Lambda) + diag(resid_h2['animal',]/tot_Eta_prec[1,]))
...
boxplot(BSFG_state$Posterior$B1[,1,],outline=F);abline(h=0)
boxplot(BSFG_state$Posterior$B2_F[,1,],outline=F);abline(h=0)
```
