...
BSFG_state = setup_model_BSFG(Y,~(1|animal),
                              # extra_regressions = NULL,
                              extra_regressions = list(X = model.matrix(~Fixed1,data)[,-1,drop=FALSE],factors=T,resids=F),
                              data,relmat = list(animal = K),
...
BSFG_state$Posterior$posteriorSample_params = BSFG_state$Posterior$posteriorSample_params[BSFG_state$Posterior$posteriorSample_params != 'cis_effects']
...
```{r}
# for(i in 1:100) {
# # BSFG_state$current_state$Lambda[1,]
# # BSFG_state$current_state$F[1,]
# # BSFG_state$current_state$resid_h2_index
# # BSFG_state$current_state$F_h2_index
#   BSFG_state$current_state = sample_latent_traits(BSFG_state,grainSize = 1)
# }
```
...
#                        run_parameters=run_parameters,
#                        priors=priors,
#                        run_ID = 'BSFG_example', # folder to store run outputs
...
G_samples = get_posterior_FUN(BSFG_state,Lambda %*% diag(F_h2['animal.(Intercept)',]) %*% t(Lambda) + diag(resid_h2['animal.(Intercept)',]/tot_Eta_prec[1,]))
