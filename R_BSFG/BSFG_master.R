                                    ...
                                    ncores = 1,simulation = F,setup = NULL,verbose=T)
{
	...
	if(sampler == 'fast_BSFG'){
		BSFG_state = fast_BSFG_ipx_sampler_init(Y, fixed, random, data, priors, run_parameters, A_mats, A_inv_mats,
												fixed_Factors, scaleY, simulation, setup, verbose)
	} else{
		BSFG_state = BSFG_discreteRandom_init(Y, fixed, random, data, priors, run_parameters, A_mats, A_inv_mats,
...
BSFG_sampler = function(BSFG_state,n_samples, ncores){
	...
	if(sampler == 'fast_BSFG'){
		...
		BSFG_state = fast_BSFG_ipx_sampler(BSFG_state,n_samples)
	} else{
		if(BSFG_state$run_parameters$verbose) print('general_BSFG sampler')
		BSFG_state = BSFG_discreteRandom_sampler(BSFG_state,n_samples, ncores)
	}
	BSFG_state$run_parameters$sampler = sampler
	return(BSFG_state)
}
