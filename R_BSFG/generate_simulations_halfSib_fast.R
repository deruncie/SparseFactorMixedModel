...
prepare_simulation2 = function(A_chol,A,Lambda,factor_h2s,E_a_prec,resid_Y_prec,name){
	n = dim(A_chol)[1]  # ??difference of A_chol and A
	p = nrow(Lambda) 
	...
	B = rep(0,p)
	Z_1 = diag(r)[,rep(1:r,n/r)]  #?? we defined r = n at 8
	Va = colSums(Lambda^2) * factor_h2s + 1/E_a_prec  #?? G
	Ve = colSums(Lambda^2) * (1-factor_h2s) + 1/resid_Y_prec # R
	...
	h2 = diag(G)/(diag(G)+diag(R))  # ?? difference with 26
...
for(size in names(sample_size)){
	...
	for(rep in 1:num_reps){
		...
		try(dir.create(dir))
		setwd(dir)
		prepare_simulation2(A_chol,A,Lambda,factor_h2s,E_a_prec,resid_Y_prec,name)
		setwd('..')
	}
}
