...
\item Normalize $\B{F}, \B{U}_{F_i}$ by post-multiplying by$\B{\Psi}^{F^{-1/2}}$ so that $\B{F}$ has unit total variance. Similarly, normalize $\B{\Lambda}$ by post-multiplying by $\B{\Psi}^{F^{1/2}}$ to keep the product $\B{F\Lambda}^T$ the same.
\end{enumerate}
