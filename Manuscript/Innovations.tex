...
\item With one random effect, the distribution of $\B{f}_j$ is: $$\B{f}_j | \B{b}_{F_j}~\sim~\mbox{N}(\B{X}_F\B{b}_{F_j},h^2_{F_j} \B{ZKZ}^T + (1-h^2)\B{I})$$. 
This means that each factor has the variance = 1 regardless of the size of $\B{X}_F\B{b}_{F_j}$. What does this mean for the corresponding magnitude of $\B{\lambda}_j$? 
\end{enumerate}
