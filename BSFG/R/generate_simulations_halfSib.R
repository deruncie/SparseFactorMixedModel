...
family_simulation = function(name = 'simulation_1', family_sizes,within_group_cor = .25,p, b, factor_h2s, prop_Factor_p = 0.5,total_h2 = 0.5,Vb = 0, numeff = NULL){
  ...
  for(h in 1:k){
    Lambda[sample(1:p,numeff[h]),h] = 1 #rnorm(numeff[h])
  }
  # cols=1:k
  # g_cols = factor_h2s>0
  ...
  V_tot = rep(1,p)
  V_factor = runif(p)
  Lambda_scale = rowSums(Lambda^2) / V_factor
  Lambda_scale[Lambda_scale == 0] = 1
  Lambda = Lambda/sqrt(Lambda_scale)
  V_resid = V_tot - rowSums(Lambda^2)
  ...
  Vg_factor = rowSums(Lambda^2 * factor_h2s)
  ...
  G = crossprod(sqrt(factor_h2s)*t(Lambda)) + diag(Vg_resid)
  R = crossprod(sqrt(1-factor_h2s)*t(Lambda)) + diag(V_resid-Vg_resid)
