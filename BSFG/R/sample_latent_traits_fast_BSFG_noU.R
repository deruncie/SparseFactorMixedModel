sample_latent_traits.fast_BSFG_noU = function(BSFG_state,grainSize,...) {
  ...
  current_state = with(c(priors,run_parameters, run_variables,data_matrices),within(current_state, {
    ...
    UtEta_prec = uncorrelated_prec_mat(resid_h2,tot_Eta_prec,s)
    UtF_prec = uncorrelated_prec_mat(F_h2,tot_F_prec,s)
    UtEta_tilde = toDense(Ut %*% (Eta - XB))
    UtF_mean = toDense(Ut %*% XFBF)
    randn_theta = matrix(rnorm(n*k),k)
    randn_e = matrix(rnorm(n*p),p)
    FtU = sample_coefMat_uncorrelated_parallel_Eigen( t(UtEta_tilde),Lambda,t(UtEta_prec), t(UtF_mean),t(UtF_prec),randn_theta,randn_e,grainSize)
    F[] = toDense(t(FtU %*% Ut))
