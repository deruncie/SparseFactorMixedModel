...
sample_tot_prec = function(Y, tot_Eta_prec_shape, tot_Eta_prec_rate, Sigma_Choleskys,Sigma_Perm, h2s_index,ncores){
	...
	scores2 = tot_prec_scores(Y,chol_Sigmas,h2s_index,1)
	microbenchmark(
...
sample_h2s_discrete_MH = function(Y,tot_Eta_prec, Sigma_Choleskys,Sigma_Perm,discrete_priors,h2s_matrix,h2_index,step_size,ncores){
	...
	h2s_index2 = sample_h2s_discrete_MH_c(Yp,h2s_matrix,chol_Sigmas,log_det_Sigmas,tot_Eta_prec,discrete_priors,r_draws,step_size,h2_index,state_draws)+1
	h2s_index3 = sample_h2s_discrete_MH_c2(Yp,h2s_matrix,chol_Sigmas,log_det_Sigmas,tot_Eta_prec,discrete_priors,r_draws,step_size,h2_index,state_draws,1)+1
...
sample_h2s_discrete = function(Y,tot_Eta_prec, Sigma_Choleskys,Sigma_Perm,discrete_priors,ncores){
	...
	log_ps2 = log_p_h2s(Y,tot_Eta_prec,chol_Sigmas,log_dets,discrete_priors,1)
	microbenchmark({
	  log_ps = mclapply(1:discrete_bins,function(i) {
	    Cholesky_Sigma = Sigma_Choleskys[[i]]$Cholesky_Sigma
	    log_det_Sigma = Sigma_Choleskys[[i]]$log_det
	...
	h2s_index2 = sample_h2s(log_ps,rs,1)
	microbenchmark(
