...
# recover()
  ...
  res = sample_MME_fixedEffects_c(Y,W,chol_Rs,h2s_index,tot_Eta_prec,prior_mean,prior_prec,randn_theta,randn_e,1)
...
sample_MME_ZKZts = function(Y, W, tot_Eta_prec, randomEffect_C_Choleskys, h2s, h2s_index, chol_Ki_mats,ncores){
	...
	theta = sample_MME_ZKZts_c(Y,W,tot_Eta_prec,chol_Cs,chol_K_invs,h2s,h2s_index,randn_theta,randn_e,1)
# recover()
	...
	# 	# res3 = sapply(traits_j,function(i) sample_MME_single_diagR_c(as.matrix(Y[,i,drop=F]), W, chol_C, pes[i], chol_K_inv_2* sqrt(tot_Eta_prec[i]),tot_Eta_prec[i], randn_theta[,i,drop=F], randn_e[,i,drop=F]))
  ...
  # plot(theta,theta2);abline(0,1)
	...
	# # sample_MME_ZKZts_c2(Y,W,tot_Eta_prec,chol_Cs,chol_K_invs,h2s,h2s_index,randn_theta,randn_e,1)
	# )
}
...
sample_tot_prec = function(Y, tot_Eta_prec_shape, tot_Eta_prec_rate, Sigma_Choleskys,Sigma_Perm, h2s_index,ncores){
	...
	scores = tot_prec_scores(Y,chol_Sigmas,h2s_index,1)
	...
	# microbenchmark(
	#   mclapply(seq_along(unique_h2s),function(j){
	#     h2_index = unique_h2s[j]
	#     Cholesky_Sigma = Sigma_Choleskys[[unique_h2s[j]]]$Cholesky_Sigma
	#     colSums(solve(Cholesky_Sigma,Yp[,unique_h2s_index[[j]]],'L')^2)
	#   },mc.cores = ncores),
	#   tot_prec_scores(Y,chol_Sigmas,h2s_index,1)
	# )
...
sample_h2s_discrete_MH = function(Y,tot_Eta_prec, Sigma_Choleskys,Sigma_Perm,discrete_priors,h2s_matrix,h2_index,step_size,ncores){
	...
	state_draws = runif(p)
	chol_Sigmas = lapply(Sigma_Choleskys,function(x) as(expand(x$Cholesky_Sigma)$L,'dgCMatrix'))
	log_det_Sigmas = sapply(Sigma_Choleskys,function(x) x$log_det)
	h2s_index = sample_h2s_discrete_MH_c(Yp,h2s_matrix,chol_Sigmas,log_det_Sigmas,tot_Eta_prec,discrete_priors,r_draws,step_size,h2_index,state_draws,1)+1
	...
	# 		# chol_Sigma = as(expand(Sigma_Choleskys[[old_state]]$Cholesky_Sigma)$L,'dgCMatrix')
	# 		# log_det_Sigma = Sigma_Choleskys[[old_state]]$log_det
	# 		# old_log_p2 <- log_prob_h2_c(Yp[,j],chol_Sigma,log_det_Sigma,n,tot_Eta_prec[j],discrete_priors[old_state])
	# 		# microbenchmark(log_prob_h2(old_state,Yp[,j],Sigma_Choleskys,n,tot_Eta_prec[j],discrete_priors[old_state]),log_prob_h2_c(Yp[,j],chol_Sigma,log_det_Sigma,n,tot_Eta_prec[j],discrete_priors[old_state]))
	#
	# 		h2_dist = colSums(abs(h2s_matrix - c(h2s_matrix[,old_state])))
	# 		candidate_new_states <- which(h2_dist < step_size & h2_dist > 0)
	# 		proposed_state = candidate_new_states[state_draws[j]*(length(candidate_new_states))+1]
	# 		# proposed_state <- sample(candidate_new_states,1)
	# 		# cs2 = candidate_states(h2s_matrix,step_size,old_state)
	#
	# 		new_log_p <- log_prob_h2(proposed_state,Yp[,j],Sigma_Choleskys,n,tot_Eta_prec[j],discrete_priors[proposed_state])
	# 		h2_dist = colSums(abs(h2s_matrix - c(h2s_matrix[,proposed_state])))
	# 		candidate_state_from_new_state <- which(h2_dist < step_size & h2_dist > 0)
	#
	...
	#   }
	#   ,
	#   sample_h2s_discrete_MH_c(Yp,h2s_matrix,chol_Sigmas,log_det_Sigmas,tot_Eta_prec,discrete_priors,r_draws,step_size,h2_index,state_draws)
	#   ,sample_h2s_discrete_MH_c2(Yp,h2s_matrix,chol_Sigmas,log_det_Sigmas,tot_Eta_prec,discrete_priors,r_draws,step_size,h2_index,state_draws,1)
	# )
}
...
sample_h2s_discrete = function(Y,tot_Eta_prec, Sigma_Choleskys,Sigma_Perm,discrete_priors,ncores){
	...
	# recover()
	chol_Sigmas = lapply(Sigma_Choleskys,function(x) as(expand(x$Cholesky_Sigma)$L,'dgCMatrix'))
	log_dets = sapply(Sigma_Choleskys,function(x) x$log_det)
	log_ps = log_p_h2s(Y,tot_Eta_prec,chol_Sigmas,log_dets,discrete_priors,1)
	rs = runif(p)
	...
	#   }
	# },
	# log_p_h2s(Y,tot_Eta_prec,chol_Sigmas,log_dets,discrete_priors,1)
	# )
