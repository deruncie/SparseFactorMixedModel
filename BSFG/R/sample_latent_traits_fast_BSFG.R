sample_latent_traits.fast_BSFG = function(BSFG_state,grainSize,...) {
  ...
  current_state = with(c(priors,run_parameters, run_variables,data_matrices),within(current_state, {
	 ...
	 # -----Sample resid_h2, tot_Eta_prec, U_R ---------------- #
		...
		QtEta_tilde = Qt %**% Eta_tilde
		resid_prec = uncorrelated_prec_mat(resid_h2,rep(1,p),s)
		scores = tot_prec_scores_c(QtEta_tilde,resid_prec)
		...
		if(is.null(h2_step_size)) {
		  log_ps = log_p_h2s_fast(QtEta_tilde, tot_Eta_prec, h2_priors_resids,s,grainSize)
		  resid_h2_index = sample_h2s(log_ps,grainSize)
		} else{
		  resid_h2_index = sample_h2s_discrete_MH_fast_c(QtEta_tilde,tot_Eta_prec,h2_priors_resids,resid_h2_index,h2s_matrix,s,h2_step_size,grainSize)+1
		}
	...
	# -----Sample Lambda and B_F ------------------ #
		...
		if(b_F > 0){
		  ...
		  if(length(QTL_columns_factors) > 0) {
		    ...
		    b_F1 = ncol(X_F1)
		    F_tilde = F - QTL_factors_Z %**% (QTL_factors_X %*% B_F[-c(1:b_F1),,drop=FALSE])
		  }
    ...
    U_F[] = sample_randomEffects_parallel_sparse_c_Eigen(F_tilde, Z, tot_F_prec, F_h2, invert_aZZt_Kinv, grainSize)
	 ...
	 # -----Sample F----------------------- #
		...
		F[] = sample_factors_scores_sparse_c_Eigen( Eta_tilde, prior_mean,Lambda,resid_Eta_prec,F_e_prec)
		# tot_F_prec[] = temp
		# F[] = setup$F
		# tot_F_prec[] = 1
