    ...
    recover()
	 ...
	 # -----Sample resid_h2, tot_Eta_prec, U_R ---------------- #
		...
		if(is.null(h2_step_size)) {
		  resid_h2_index = sample_h2s_discrete_fast(QtEta_tilde, tot_Eta_prec, h2_priors_resids,s,grainSize)
		} else{
		  r_draws = runif(p)
		  state_draws = runif(p)
		  resid_h2_index = sample_h2s_discrete_MH_fast_c(QtEta_tilde,tot_Eta_prec,h2_priors_resids,resid_h2_index,h2s_matrix,s,r_draws,state_draws,h2_step_size,grainSize)+1
		}
