sample_factor_model.general_BSFG = function(BSFG_state,ncores = detectCores(),...) {
	...
	current_state = with(c(priors,run_parameters, run_variables,data_matrices),within(current_state, {
	 ...
	 # -----Sample tot_Eta_prec, resid_h2, U_R ---------------- #
		...
		recover()
		candidate_states = generate_candidate_states(h2s_matrix,step_size=0.2)
		resid_h2_index = sample_h2s_discrete(Eta_tilde,tot_Eta_prec, Sigma_Choleskys, Sigma_Perm, Resid_discrete_priors,ncores)
		resid_h2_index2 = sample_h2s_discrete_MH(Eta_tilde,tot_Eta_prec, Sigma_Choleskys,Resid_discrete_priors,h2s_matrix,resid_h2_index,candidate_states,ncores)

		microbenchmark(sample_h2s_discrete(Eta_tilde,tot_Eta_prec, Sigma_Choleskys, Sigma_Perm, Resid_discrete_priors,ncores),
		               sample_h2s_discrete_MH(Eta_tilde,tot_Eta_prec, Sigma_Choleskys,Resid_discrete_priors,h2s_matrix,resid_h2_index,candidate_states,1),
		               times= 10
		               )
