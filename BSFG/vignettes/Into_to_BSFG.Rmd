...
Load the simulated data. Replace with your own data.
...
priors = BSFG_priors(
  ...
  B2_prior = list(           # prior for fixed effects. Supersedes fixed_var above.
    sampler = sample_B_prec_ARD,  # class of priors. ARD is default
    global   = list(V = 1,nu = 3),  # global shrinkage hyperparameters
    global_F = list(V = 1,nu = 3), # global shrinkage hyperparameters for factors
...
1. `BSFG_init`: Specify the model through a model formula similar to `lme4`
2. `BSFG_init2`: Directly pass the model matrices

### Using `BSFG_init`
```{r}
BSFG_state = BSFG_init(Y,
                       model=~Fixed1+(1+Fixed1|animal), # This model has one fixed and two random terms. The random terms are NOT correlated
...
### Using `BSFG_init2`
```{r}
X = X_F = model.matrix(~Fixed1,data)
# random effect 1: intercept for each animal
Z = model.matrix(~0+animal,data)
# ensure that Z is ordered the same as K
colnames(Z) = sub('animal','',colnames(Z))
Z = Z[,rownames(K)]

# random effect 2: slope on Fixed1 for each animal
Z2 = Z * data$Fixed1

# RE_setup list:
# each (named) element (one per random effect) contains: 
...
RE_setup = list(`animal.(Intercept)` = list(Z = Z, K = K), animal.Fixed1 = list(Z = Z2, K = K))
BSFG_state = BSFG_init2(Y = Y, X = X, X_F = X_F,
                       RE_setup = RE_setup,
                       data = data, # the data.frame with information for constructing the model matrices
                       run_parameters=run_parameters,
                       priors=priors,
